...
1. 1 <= N <= 100
2. Each line contains a maximum of 10^4 printable characters.
3. The total number of characters in all test cases will not exceed 10^6.

# Output Format

For each line, print the content enclosed within valid tags.
If a line contains multiple instances of valid content, print out each instance of valid content
on a new line; if no valid content is found, print None.
...
public class TagContentExtractor {
    ...
    public List<String> extract(String input) {
        int startIdx = findOpening(input);
        int endIdx = findClosing(input, startIdx);
        int nestedIdx = findOpening(input, startIdx + 2);
        String result = input.substring(startIdx, endIdx);
        return List.of(result);
    }

    private int findClosing(String input, int fromIdx) {
        return input.indexOf("</" + currentTag + ">", fromIdx);
    }

    private int findOpening(String input) {
        return findOpening(input, 0);
    }

    private int findOpening(String input, int from) {
        ...
        currentTag = input.substring(i + 1, j);
        return j + 1;
    }
